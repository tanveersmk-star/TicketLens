...
<head>
  ...
  <meta http-equiv="Content-Security-Policy"
    content="default-src 'none'; script-src 'self' https://cdnjs.cloudflare.com https://cdn.jsdelivr.net 'unsafe-inline'; style-src 'self' https://fonts.googleapis.com 'unsafe-inline'; font-src https://fonts.gstatic.com; connect-src https:; img-src 'self' data: blob:; worker-src https://cdnjs.cloudflare.com blob:; object-src 'none'; frame-src 'none'; base-uri 'self'; form-action 'none';">
  <title>ITSM Ticket Intelligence Hub</title>
  <script src="https://cdnjs.cloudflare.com/ajax/libs/pdf.js/2.16.105/pdf.min.js"></script>
  <script>pdfjsLib.GlobalWorkerOptions.workerSrc = 'https://cdnjs.cloudflare.com/ajax/libs/pdf.js/2.16.105/pdf.worker.min.js';</script>
  <script src="config.js"></script>
  <script src="https://cdn.jsdelivr.net/npm/mermaid@10/dist/mermaid.min.js"></script>
  <script>mermaid.initialize({ startOnLoad: false, theme: 'base', themeVariables: { primaryColor: '#f4f5f7', primaryTextColor: '#1a1a2e', primaryBorderColor: '#009999', lineType: 'basis' } });</script>
  ...
  <style>
    :root {
      ...
      --success: #00a86b;
      --success-bg: #e8f9f0;
      --success-border: #7ddcaa;
      --warning: #e69500;
      --warning-bg: #fef7e6;
      --warning-border: #fad47a;
      --danger: #d63d2f;
      --danger-bg: #fdeeed;
      --danger-border: #f5b8b3;
      --info: #0078b4;
      --info-bg: #e6f3fa;
      --info-border: #9fd3f0;
      --purple: #6f5091;
      --purple-bg: #f3f0f7;
      --purple-border: #c9b8de;
      --teal: #009999;
      --teal-bg: #e6f7f7;
      --teal-border: #80d4d4;
      --header-dark: #1a1a2e;
    ...
    * {
    ...
    .app-header::before {
      ...
      right: 0;
      bottom: 0;
      background: linear-gradient(135deg, var(--header-dark) 0%, var(--header-mid) 50%, #1e3a50 100%);
    }
    ...
    .app-header::after {
      ...
      right: 0;
      height: 3px;
      background: linear-gradient(90deg, var(--primary) 0%, var(--accent) 50%, #00e6e6 100%);
    }
    ...
    .dropzone {
      ...
      padding: 40px 20px;
      text-align: center;
      background: linear-gradient(180deg, #fafbfc 0%, var(--primary-light) 100%);
      cursor: pointer;
      transition: all 0.2s;
    }
    ...
    .vtl::before {
      ...
      left: 11px;
      width: 2px;
      background: linear-gradient(180deg, var(--primary) 0%, var(--info) 50%, var(--success) 100%);
    }
    ...
    .sec-shield-icon { font-size: 1.4rem; flex-shrink: 0; }
    .sec-dismiss-btn {
      margin-left: auto;
      background: none;
    ...
    .sec-dismiss-btn:hover { background: #fee2e2; }
    ...
    /* ── Timeline Table ── */
    .tl-table { width:100%; border-collapse:collapse; font-size:0.82rem; }
    .tl-table th {
      background:var(--header-dark); color:white; padding:9px 11px;
      text-align:left; font-size:0.69rem; text-transform:uppercase;
      letter-spacing:0.05em; white-space:nowrap; font-weight:700;
      position:sticky; top:0; z-index:2;
    }
    .tl-table td { padding:8px 10px; border-bottom:1px solid var(--border-light); vertical-align:top; }
    .tl-row:hover td { background:#f0f7ff !important; }
    .tl-row-sentiment td { background:var(--danger-bg); }
    .tl-row-sentiment td:first-child { border-left:4px solid var(--danger); }
    .tl-row-priority td { background:var(--warning-bg); }
    .tl-row-priority td:first-child { border-left:4px solid var(--warning); }
    .tl-row-noaction td { color:var(--text-light); font-style:italic; background:#fafafa; }
    .tl-row-created td { background:var(--teal-bg); }
    .tl-row-created td:first-child { border-left:4px solid var(--teal); }
    .tl-row-resolved td { background:var(--success-bg); }
    .tl-row-resolved td:first-child { border-left:4px solid var(--success); }
    .tl-date { white-space:nowrap; font-weight:700; color:var(--primary-dark); font-size:0.79rem; }
    .tl-day  { white-space:nowrap; font-size:0.72rem; color:var(--text-muted); }
    .tl-time { white-space:nowrap; font-family:'JetBrains Mono',monospace; font-size:0.77rem; color:var(--text-secondary); }
    .tl-summary { max-width:320px; line-height:1.55; }
    .tl-actor { max-width:120px; font-size:0.79rem; word-break:break-word; }
    .tl-change { white-space:nowrap; font-size:0.77rem; font-family:'JetBrains Mono',monospace; }
    .tl-status { white-space:nowrap; font-size:0.79rem; }
    .tl-priority { white-space:nowrap; }
    .tl-overflow { overflow-x:auto; border-radius:var(--radius-sm); max-height:600px; overflow-y:auto; box-shadow:var(--shadow-sm); }
    .evt-badge {
      display:inline-block; padding:2px 7px; border-radius:3px;
      font-size:0.63rem; font-weight:700; letter-spacing:0.04em;
      text-transform:uppercase; white-space:nowrap;
    }
    .evt-created  { background:#d1fae5; color:#065f46; }
    .evt-state    { background:var(--purple-bg); color:#4a2d6e; }
    .evt-group    { background:var(--info-bg); color:#005a8c; }
    .evt-person   { background:var(--info-bg); color:#005a8c; }
    .evt-priority { background:var(--warning-bg); color:#8a5a00; }
    .evt-comment  { background:#f0f0f0; color:#555; }
    .evt-email    { background:var(--teal-bg); color:#006666; }
    .evt-noaction { background:#f5f5f5; color:#bbb; font-style:italic; }
    .evt-resolved { background:var(--success-bg); color:#0a6b42; }
    .evt-other    { background:#f0f0f0; color:#555; }
    .prio-badge {
      display:inline-block; padding:2px 8px; border-radius:3px;
      font-size:0.67rem; font-weight:700; white-space:nowrap;
    }
    .prio-critical { background:#fee2e2; color:#7f1d1d; border:1px solid #fca5a5; }
    .prio-high     { background:var(--danger-bg); color:#9b2318; border:1px solid var(--danger-border); }
    .prio-medium   { background:var(--warning-bg); color:#8a5a00; border:1px solid var(--warning-border); }
    .prio-low      { background:var(--success-bg); color:#0a6b42; border:1px solid var(--success-border); }
    .prio-default  { background:var(--border-light); color:var(--text-muted); border:1px solid var(--border); }
    .sentinel-banner {
      ...
      font-size:0.86rem; font-weight:600; color:#9b2318;
    }
    .inc-grid {
    ...
    .prio-track-pill {
      ...
      border-radius:99px; font-size:0.8rem; font-weight:600; color:#8a5a00;
      margin-bottom:4px;
    }


    /* ── Slight global font reduction ── */
    /* applied via body below */

    /* ── Event Card Timeline ── */
    .evt-day-block { margin-bottom: 20px; }
    .evt-day-header {
      display:flex; align-items:center; justify-content:space-between;
      padding:6px 13px; margin-bottom:10px;
      background:var(--teal-bg); border-left:4px solid var(--primary);
      border-radius:0 5px 5px 0;
    }
    .evt-day-date-big { font-size:0.87rem; font-weight:800; color:var(--primary-dark); }
    .evt-day-name-text { font-size:0.67rem; font-weight:700; color:var(--text-muted); text-transform:uppercase; letter-spacing:0.05em; margin-top:1px; }
    .evt-day-badge { font-size:0.67rem; font-weight:600; white-space:nowrap; padding:2px 8px; border-radius:10px; }
    .evt-weekend { color:var(--purple); background:var(--purple-bg); }
    .evt-workday { color:var(--success); background:var(--success-bg); }
    .evt-items-container { position:relative; padding-left:32px; }
    .evt-items-container::before {
      content:''; position:absolute;
      left:7px; top:6px; bottom:6px; width:2px;
      background:linear-gradient(180deg, var(--teal-border) 0%, var(--border-light) 100%);
      border-radius:1px;
    }
    .evt-card {
    ...
    .evt-card:hover { box-shadow:var(--shadow); transform:translateX(2px); }
    .evt-card::before {
      content:''; position:absolute;
      left:-25px; top:10px;
      width:13px; height:13px;
      border-radius:50%;
      background:var(--dot-clr, var(--info));
      border:2px solid white;
      box-shadow:0 0 0 2px var(--dot-clr, var(--info));
    }
    .evt-card-sentiment  { border-left:3px solid var(--danger); background:#fff5f5; }
    .evt-card-sentiment::before  { left:-27px; }
    .evt-card-priority   { border-left:3px solid var(--warning); background:#fffbf3; }
    .evt-card-priority::before   { left:-27px; }
    .evt-card-created    { border-left:3px solid var(--teal); background:var(--teal-bg); }
    .evt-card-created::before    { left:-27px; }
    .evt-card-resolved   { border-left:3px solid var(--success); background:var(--success-bg); }
    .evt-card-resolved::before   { left:-27px; }
    .evt-card-noaction   { border:1px dashed var(--border); background:#fafafa; opacity:0.75; }
    .evt-header-row { display:flex; align-items:center; flex-wrap:wrap; gap:5px; margin-bottom:5px; }

    .evt-day-header.clickable { cursor:pointer; user-select:none; }
    .evt-day-header.clickable:hover { filter:brightness(0.95); }
    .evt-collapsed .evt-items-container { display:none; }
    .evt-toggle-icon { font-size:0.75rem; margin-right:5px; transition:transform 0.2s; display:inline-block; }
    .evt-collapsed .evt-toggle-icon { transform:rotate(-90deg); }
    .evt-time-tag {
    ...
    .evt-body-text { font-size:0.8rem; color:var(--text-secondary); line-height:1.5; }
    .evt-narrative-row {
      margin-top:8px;
      padding:10px 12px;
      background:var(--surface);
    ...
    .evt-change-row {
      margin-top:5px; font-size:0.71rem;
      font-family:'JetBrains Mono', monospace; color:var(--text-muted);
      background:var(--bg); padding:3px 8px; border-radius:3px; display:inline-block;
    }
    .evt-actor-row { margin-top:4px; font-size:0.7rem; color:var(--text-light); }
    .evt-sentiment-tag {
      font-size:0.63rem; font-weight:700; padding:2px 6px;
      background:var(--danger); color:white; border-radius:3px;
    ...
    .evt-noaction-dash { font-family:monospace; color:var(--border); }
    .evt-noaction-label { font-weight:700; color:var(--text-muted); }
    .evt-noaction-dayname { color:var(--text-light); }
    .evt-noaction-msg { font-style:italic; color:var(--text-light); }
    ...
    .role-map-name { font-weight: 600; color: var(--primary-dark); font-size: 0.9rem; }
    .role-map-select {
    ...
    .role-map-select:focus { border-color: var(--primary); }
    .btn-run {
    ...
    .btn-run:hover { background:#0ea5e9; } /* fallback hover */
    ...
    .btn-reset svg { width: 14px; height: 14px; flex-shrink: 0; }
    ...
    .model-pill input[type="radio"]:checked + label {
      border-color: var(--primary);
      background: linear-gradient(135deg, var(--primary-light) 0%, #d0f2f2 100%);
      color: var(--primary-dark);
      box-shadow: 0 0 0 3px rgba(0,153,153,0.15), var(--shadow);
    }
    .model-pill-icon {
      width: 26px;
      height: 26px;
    ...
    .model-pill-name { font-weight: 700; font-size: 0.86rem; }
    .model-pill-sub { font-size: 0.67rem; color: var(--text-light); font-weight: 500; }
    .model-pill input[type="radio"]:checked + label .model-pill-sub {
      color: var(--primary);
    }
    .model-no-selection-hint {
      font-size: 0.76rem;
      color: var(--text-light);
      margin-top: 8px;
      font-style: italic;
    ...
    /* ── Re-analyze Toolbar (shown above results) ── */
    #reanalyze-toolbar {
      display: none;
      margin-bottom: 16px;
      padding: 12px 18px;
      background: linear-gradient(135deg, #f0fafa 0%, #e6f3ff 100%);
      border: 1px solid var(--teal-border);
      border-left: 4px solid var(--primary);
      border-radius: var(--radius);
      box-shadow: var(--shadow-sm);
    ...
    .toolbar-model-btn:hover {
      border-color: var(--primary);
      color: var(--primary-dark);
      background: var(--primary-light);
    }
    .toolbar-model-btn.active-model {
      border-color: var(--primary);
      background: var(--primary);
      color: white;
      cursor: default;
    ...
    .btn-reanalyze svg { width: 14px; height: 14px; }
    ...
    .btn-admin-trigger:hover { background: var(--primary); color: white; border-color: var(--primary); }
    .btn-admin-trigger svg { width: 12px; height: 12px; }
    ...
    .active-model-info { flex: 1; min-width: 0; }
    .active-model-label {
      font-size: 0.62rem; font-weight: 700; text-transform: uppercase;
      letter-spacing: 0.07em; color: var(--text-light); margin-bottom: 1px;
    }
    .active-model-name {
    ...
    /* ── Test result panel (full width, readable) ── */
    .model-test-result {
      display: none;
      padding: 10px 16px;
      border-top: 1px solid var(--border-light);
    ...
    .model-test-result.ok  { display: block; background: var(--success-bg); color: #065f46; }
    .model-test-result.err { display: block; background: #fff1f2; color: #9b1c1c; border-left: 4px solid var(--danger); }
    ...
    .btn-set-default:hover { background: var(--primary); color: white; }
    ...
    .admin-close-btn:hover { background: var(--danger); border-color: var(--danger); }
    ...
    .btn-admin-add:hover { background: var(--primary-dark); }
    .btn-admin-secondary {
      display: inline-flex; align-items: center; gap: 5px;
    ...
    .btn-admin-secondary:hover { border-color: var(--primary); color: var(--primary-dark); }
    ...
    .model-admin-card:hover { box-shadow: var(--shadow); }
    .model-admin-card-main {
      display: grid;
    ...
    .model-admin-label { font-weight: 700; font-size: 0.9rem; color: var(--text); }
    .model-admin-sub  { font-size: 0.72rem; color: var(--text-muted); margin-top: 1px; font-family: 'JetBrains Mono', monospace; }
    .model-admin-url  { font-size: 0.72rem; color: var(--info); word-break: break-all; margin-top: 2px; }
    .model-admin-key-row {
      display: flex; align-items: center; gap: 6px;
    }
    .model-admin-key-text {
      font-family: 'JetBrains Mono', monospace;
    ...
    .model-admin-key-text.no-key { color: var(--danger); font-style: italic; }
    .btn-eye {
      background: none; border: 1px solid var(--border);
      border-radius: 4px; color: var(--text-muted); cursor: pointer;
      padding: 3px 6px; font-size: 0.68rem; transition: all 0.12s;
      flex-shrink: 0;
    }
    .btn-eye:hover { background: var(--bg); border-color: var(--primary); color: var(--primary); }
    .model-admin-actions { display: flex; gap: 6px; align-items: center; flex-shrink: 0; }
    .btn-admin-action {
      padding: 5px 10px; font-size: 0.71rem; font-weight: 600;
      border-radius: var(--radius-sm); cursor: pointer; transition: all 0.12s;
      border: 1px solid var(--border); background: var(--surface); color: var(--text-muted);
    }
    .btn-admin-action:hover { border-color: var(--primary); color: var(--primary-dark); background: var(--primary-light); }
    .btn-admin-action.del { border-color: var(--danger-border); color: var(--danger); background: var(--danger-bg); }
    .btn-admin-action.del:hover { background: var(--danger); color: white; }
    .model-test-badge {
      font-size: 0.65rem; font-weight: 700; padding: 2px 8px; border-radius: 99px;
      display: none;
    }
    .model-test-badge.ok  { display: inline; background: var(--success-bg); color: var(--success); border: 1px solid var(--success-border); }
    .model-test-badge.err { display: inline; background: var(--danger-bg); color: var(--danger); border: 1px solid var(--danger-border); }
    .model-test-badge.spin { display: inline; background: var(--teal-bg); color: var(--primary-dark); border: 1px solid var(--teal-border); }

    /* Edit / Add Form inside admin */
    .model-admin-editform {
      display: none;
      padding: 14px 16px;
      background: var(--bg);
      border-top: 1px solid var(--border-light);
    }
    .model-admin-editform.open { display: block; }
    .admin-form-grid {
      display: grid;
    ...
    .admin-field label {
      ...
      letter-spacing: 0.06em; color: var(--text-light); display: block; margin-bottom: 4px;
    }
    .admin-field input {
    ...
    .admin-form-actions { margin-top: 12px; display: flex; gap: 8px; }
    .btn-admin-save {
      padding: 7px 16px; font-size: 0.78rem; font-weight: 700;
      border: none; border-radius: var(--radius-sm);
      background: var(--success); color: white; cursor: pointer; transition: background 0.15s;
    }
    .btn-admin-save:hover { background: #008056; }
    .btn-admin-cancel {
    ...
    /* Add-new form at bottom of admin */
    .admin-addform-section {
    ...
    .admin-addform-section.open { display: block; }
    .admin-addform-title {
    ...
    .header-model-chip::before {
      content: '';
      position: absolute; inset: -1px;
      border-radius: 13px;
      background: linear-gradient(135deg, rgba(0,166,126,0.5), rgba(91,106,240,0.3));
      z-index: -1;
      opacity: 0;
      transition: opacity 0.2s;
    }
    .header-model-chip:hover { background: rgba(255,255,255,0.12); border-color: rgba(0,166,126,0.6); }
    .header-model-chip:hover::before { opacity: 1; }
    .hmc-icon {
    ...
    .hmc-info { line-height: 1.3; min-width: 0; }
    .hmc-label {
      font-size: 0.58rem; font-weight: 700; text-transform: uppercase;
      letter-spacing: 0.1em; color: rgba(255,255,255,0.45); margin-bottom: 1px;
    }
    .hmc-name {
      font-size: 0.88rem; font-weight: 700; color: rgba(255,255,255,0.95);
      white-space: nowrap;
    }
    .hmc-sub {
    ...
    .header-model-chip:hover .hmc-gear { color: rgba(0,230,150,0.9); }
    .hmc-gear svg { transition: transform 0.3s; }
    .header-model-chip:hover .hmc-gear svg { transform: rotate(60deg); }

  
    .skill-card { background: #161b22; border: 1px solid #30363d; border-radius: 8px; padding: 16px; cursor: pointer; transition: all 0.2s; }
...
<body>
  ...
  <div class="app-header">
    <div class="app-header-inner" style="display:flex;align-items:center;gap:24px;">
      <div class="header-accent"></div>
      <div style="flex:1;">
        <h1>ITSM Ticket Intelligence Hub</h1>
        <p class="subtitle">OmniTracker PDF Analysis — Service Manager CSAT Review & Technical Root Cause</p>
        <p style="font-size: 0.72rem; color: var(--text-light); text-transform: uppercase; margin-top: 8px; letter-spacing: 0.5px; opacity: 0.8;">
           ⚠️ Disclaimer: All intelligence is AI-generated and subject to hallucination. Human review required for reporting.
        </p>
      </div>
      <!-- Fancy Model Status Chip top-right -->
      <div class="header-model-chip" id="header-model-chip" onclick="openAdmin()" title="Click to configure AI models">
        <span class="hmc-icon" id="hmc-icon" style="background:linear-gradient(135deg,#00a67e,#007a5e);">AI</span>
        <div class="hmc-info">
          <div class="hmc-label">Active Model</div>
          <div class="hmc-name" id="hmc-name">Loading…</div>
          <div class="hmc-sub" id="hmc-sub">—</div>
        </div>
        <div class="hmc-gear">
  ...
  <!-- Security Warning Banner (hidden by default, shown when threats detected) -->
  <div id="sec-warning-banner">
    <div style="display:flex;align-items:flex-start;gap:12px;max-width:1200px;margin:0 auto;">
      <span class="sec-shield-icon">🛡️</span>
      <div style="flex:1;">
        <strong style="color:#7f1d1d;font-size:0.92rem;" id="sec-warn-title">Security Alert: Suspicious Content Detected</strong>
        <div style="margin-top:4px;font-size:0.82rem;color:#991b1b;" id="sec-warn-body">
          Patterns associated with shell execution or social engineering were found.
          Do <strong>not</strong> open PowerShell, Command Prompt, or any terminal based on content from this analysis.
        </div>
        <div style="margin-top:5px;font-size:0.78rem;color:#7f1d1d;" id="sec-warn-detail"></div>
      </div>
      <button class="sec-dismiss-btn" onclick="document.getElementById('sec-warning-banner').style.display='none'">Dismiss</button>
    </div>
  </div>
  ...
  <div class="container">
    ...
    <div class="upload-panel">
      <div class="upload-panel-header">
        <span class="upload-panel-title">📄 PDF Source</span>
        <button class="btn-reset" id="btn-reset" onclick="resetToNewFile()" title="Load a new PDF file">
      ...
      <div class="dropzone" id="dropzone">
        <svg class="dz-icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5"
            d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
        </svg>
        <p>Drop OmniTracker PDF export here</p>
        <button class="btn-browse" id="btn-browse">Select PDF</button>
        <input type="file" id="file-input" accept=".pdf">
        <div class="hint">PDF is read client-side → AI analyzes via API → No data stored</div>
      </div>

      <div id="file-status" class="file-status"></div>
    </div>

    <!-- Re-analyze Toolbar (shown after analysis completes) -->
    <div id="reanalyze-toolbar">
      <div class="toolbar-row">
        <div class="toolbar-file-info">
          <div class="toolbar-file-name" id="toolbar-filename">—</div>
          <div class="toolbar-model-badge">
            <span class="badge-dot"></span>
            <span id="toolbar-model-label">Analyzed with —</span>
          </div>
        </div>
        <div class="toolbar-model-pills" id="toolbar-model-pills"></div>
        <button class="btn-reanalyze" id="btn-reanalyze" onclick="triggerReanalyze()" disabled>
          <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <!-- Role Mapping Panel -->
    <div id="role-mapping-panel" style="display:none;">
      <h3 style="margin-bottom:8px; color:var(--primary-dark);">👤 Identify Key Users (Human-in-the-Loop)</h3>
      <p style="font-size:0.85rem; color:var(--text-secondary); margin-bottom:15px;">
        To prevent AI hallucinations (e.g. confusing a Service Team member's operational follow-up with 'Customer Frustration'), please review the roles assigned by the AI below. Adjust them if necessary, then run the full analysis.
      </p>
      <div id="role-mapping-list"></div>
      <button class="btn-run" id="btn-run-analysis" onclick="runFullAnalysis()">▶ Run Full Analysis</button>
    </div>
    ...
    <div class="progress-panel" id="progress-panel">
      <div class="progress-steps">
        <div class="step-item" id="step-extract"><span class="step-spinner"></span> Extracting PDF text...</div>
        <div class="step-item" id="step-mapping"><span class="step-spinner"></span> Analyzing roles...</div>
        <div class="step-item" id="step-review"><span class="step-spinner"></span> Structuring & translating
          content...</div>
        <div class="step-item" id="step-sm"><span class="step-spinner"></span> AI: Service Manager analysis...
        </div>
        <div class="step-item" id="step-arch"><span class="step-spinner"></span> AI: Technical / Architecture
          analysis...</div>
      </div>
    </div>
    ...
    <div id="results-area" style="display:none;">
      <div class="tab-bar">
        <button class="tab-btn active" data-tab="sm" onclick="switchTab('sm')">
      ...
      <div class="tab-content active" id="tab-sm"></div>
      <div class="tab-content" id="tab-review"></div>
      <div class="tab-content" id="tab-arch"></div>
    </div>
  </div>

  <!-- ─── Admin Model Configuration Panel ─── -->
  
  <!-- ─── Admin Fullscreen Panel ─── -->
  <div id="admin-overlay" class="admin-overlay" style="display:none; position:fixed; top:0; left:0; right:0; bottom:0; background:#0d1117; color:#c9d1d9; z-index:9999; overflow-y:auto; font-family:'Inter',sans-serif;">
    
    <div class="admin-top-nav" style="display:flex; align-items:center; padding:16px 24px; background:#161b22; border-bottom:1px solid #30363d; font-size:0.9rem;">
      <div class="admin-logo-box" style="background:#00cccc; color:#000; font-weight:800; border-radius:4px; padding:4px 8px; margin-right:12px;">TI</div>
      <div style="color:#8b949e;">/</div>
      <div style="color:#fff; font-weight:600; margin-left:12px; margin-right:24px;">Admin</div>
      
      <div class="admin-nav-links" style="display:flex; gap:8px;">
        <div class="admin-nav-link" id="nav-btn-models" onclick="switchAdminTabV2('models')" style="cursor:pointer; padding:6px 12px; border-radius:6px;">AI Models</div>
        <div class="admin-nav-link" id="nav-btn-skills" onclick="switchAdminTabV2('skills')" style="cursor:pointer; padding:6px 12px; border-radius:6px;">SKILL Prompts</div>
        <div class="admin-nav-link" id="nav-btn-roles" onclick="switchAdminTabV2('roles')" style="cursor:pointer; padding:6px 12px; border-radius:6px;">ITSM Roles</div>
      </div>
      
      <div class="admin-close-nav" onclick="closeAdmin()" style="margin-left:auto; color:#8b949e; cursor:pointer;">&larr; Back to Incidents</div>
    </div>

    <div class="admin-body" style="padding:32px 48px; max-width:1400px; margin:0 auto;">
      
      <!-- AI Models Tab -->
      <div id="admin-tab-models" class="admin-view" style="display:none;">
        <h2 style="color:#fff; margin-bottom:8px;">AI Models</h2>
        <p style="color:#8b949e; margin-bottom:24px; font-size:0.9rem;">Configure API endpoints and keys for local or cloud AI models.</p>
        <div style="margin-bottom:16px; display:flex; gap:10px;">
          <button style="background:#21262d; border:1px solid #30363d; color:#c9d1d9; padding:8px 16px; border-radius:6px; cursor:pointer;" onclick="toggleAddForm()">+ Add New Model</button>
          <button style="background:#21262d; border:1px solid #30363d; color:#c9d1d9; padding:8px 16px; border-radius:6px; cursor:pointer;" onclick="adminResetDefaults()">Reset Defaults</button>
        </div>
        <div id="admin-model-list"></div>
        <div class="admin-addform-section" id="admin-addform" style="display:none; background:#161b22; border:1px solid #30363d; padding:24px; border-radius:8px; margin-top:24px;">
           <h3 style="color:#fff; margin-bottom:16px;">Add New Model</h3>
           <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px; margin-bottom:24px;">
              <div>
                 <label style="display:block; color:#8b949e; margin-bottom:6px; font-size:0.8rem;">Provider Label</label>
                 <input type="text" id="new-label" style="width:100%; background:#0d1117; border:1px solid #30363d; color:#fff; padding:8px 12px; border-radius:6px;">
              </div>
              <div>
                 <label style="display:block; color:#8b949e; margin-bottom:6px; font-size:0.8rem;">Model ID</label>
                 <input type="text" id="new-modelid" style="width:100%; background:#0d1117; border:1px solid #30363d; color:#fff; padding:8px 12px; border-radius:6px;">
              </div>
              <div style="grid-column: span 2;">
                 <label style="display:block; color:#8b949e; margin-bottom:6px; font-size:0.8rem;">API Endpoint URL</label>
                 <input type="text" id="new-url" style="width:100%; background:#0d1117; border:1px solid #30363d; color:#fff; padding:8px 12px; border-radius:6px;">
              </div>
              <div style="grid-column: span 2;">
                 <label style="display:block; color:#8b949e; margin-bottom:6px; font-size:0.8rem;">API Key</label>
                 <input type="password" id="new-key" style="width:100%; background:#0d1117; border:1px solid #30363d; color:#fff; padding:8px 12px; border-radius:6px;">
              </div>
              <div>
                 <label style="display:block; color:#8b949e; margin-bottom:6px; font-size:0.8rem;">Icon Text</label>
                 <input type="text" id="new-icon" style="width:100%; background:#0d1117; border:1px solid #30363d; color:#fff; padding:8px 12px; border-radius:6px;">
              </div>
              <div>
                 <label style="display:block; color:#8b949e; margin-bottom:6px; font-size:0.8rem;">Icon Color</label>
                 <input type="text" id="new-color" style="width:100%; background:#0d1117; border:1px solid #30363d; color:#fff; padding:8px 12px; border-radius:6px;" value="#7c3aed">
              </div>
           </div>
           <button style="background:#58a6ff; color:#0d1117; padding:8px 16px; border:none; border-radius:6px; font-weight:600; cursor:pointer;" onclick="adminSaveNewModel()">Save & Apply</button>
           <button style="background:transparent; color:#8b949e; padding:8px 16px; border:none; cursor:pointer;" onclick="toggleAddForm()">Cancel</button>
        </div>
      </div>

      <!-- SKILL Prompts Tab -->
      <div id="admin-tab-skills" class="admin-view" style="display:none;">
        <h2 style="color:#fff; margin-bottom:8px;">SKILL Prompts</h2>
        <p style="color:#8b949e; margin-bottom:24px; font-size:0.9rem;">Edit the system prompt content injected into AI calls for each view. Changes take effect immediately on the next analysis run.</p>
        ...
        <div style="display:grid; grid-template-columns:300px 1fr; gap:32px;">
           ...
           <div style="display:flex; flex-direction:column; gap:12px;">
              <div class="skill-card active" id="scard-ea" onclick="selectSkill('ea')">
                 <h3 style="margin:0 0 4px; font-size:0.95rem; color:#c9d1d9;">Enterprise Architect Analysis</h3>
                 <p style="margin:0; font-size:0.8rem; color:#8b949e;">Enterprise Architect view only</p>
                 <p style="margin:4px 0 0; font-size:0.75rem; color:#8b949e;">Updated <span id="ts-ea"></span></p>
              </div>
              <div class="skill-card" id="scard-sm" onclick="selectSkill('sm')">
                 <h3 style="margin:0 0 4px; font-size:0.95rem; color:#c9d1d9;">Service Manager (CSAT)</h3>
                 <p style="margin:0; font-size:0.8rem; color:#8b949e;">Service Manager view only</p>
                 <p style="margin:4px 0 0; font-size:0.75rem; color:#8b949e;">Updated <span id="ts-sm"></span></p>
              </div>
              <div class="skill-card" id="scard-itsm" onclick="selectSkill('itsm')">
                 <h3 style="margin:0 0 4px; font-size:0.95rem; color:#c9d1d9;">ITSM Expert (Applied to All Views)</h3>
                 <p style="margin:0; font-size:0.8rem; color:#8b949e;">All views</p>
                 <p style="margin:4px 0 0; font-size:0.75rem; color:#8b949e;">Updated <span id="ts-itsm"></span></p>
              </div>
           </div>
           ...
           <div>
              <div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:16px;">
                 <div>
                    <h3 id="editor-title" style="margin:0 0 4px; color:#fff;">Enterprise Architect Analysis</h3>
                    <p id="editor-sub" style="margin:0; color:#8b949e; font-size:0.85rem;">Used in: Enterprise Architect view only</p>
                 </div>
                 <div style="display:flex; align-items:center;">
                    <span id="skill-save-msg" style="color:#3fb950; font-size:0.85rem; margin-right:12px; opacity:0; transition:opacity 0.2s;">Saved</span>
                    <button style="background:#58a6ff; color:#0d1117; padding:8px 16px; border:none; border-radius:6px; font-weight:600; cursor:pointer;" onclick="saveCurrentSkill()">Save Changes</button>
                 </div>
              </div>
              <textarea id="skill-editor" style="width:100%; height:600px; background:#0d1117; border:1px solid #30363d; border-radius:8px; color:#c9d1d9; font-family:'JetBrains Mono', Consolas, monospace; font-size:0.85rem; padding:16px; resize:vertical; line-height:1.5; outline:none;"></textarea>
           </div>
        </div>
      </div>
      ...
      <div id="admin-tab-roles" class="admin-view" style="display:none;">
        <h2 style="color:#fff; margin-bottom:8px;">ITSM Roles</h2>
        <p style="color:#8b949e; margin-bottom:24px; font-size:0.9rem;">Roles presented during the actor-mapping step. <strong>Sentiment</strong> controls whether the AI scores the emotional tone of COMM entries from actors in that role &mdash; disable for purely technical groups.</p>
        
        <table style="width:100%; border-collapse:collapse; margin-top:16px;" id="roles-table">
          <thead>
            <tr>
              <th style="text-align:left; padding:12px 16px; color:#8b949e; font-size:0.75rem; text-transform:uppercase; letter-spacing:0.05em; border-bottom:1px solid #30363d;">Role</th>
              <th style="text-align:left; padding:12px 16px; color:#8b949e; font-size:0.75rem; text-transform:uppercase; letter-spacing:0.05em; border-bottom:1px solid #30363d;">Description</th>
              <th style="text-align:left; padding:12px 16px; color:#8b949e; font-size:0.75rem; text-transform:uppercase; letter-spacing:0.05em; border-bottom:1px solid #30363d;">Order</th>
              <th style="text-align:left; padding:12px 16px; color:#8b949e; font-size:0.75rem; text-transform:uppercase; letter-spacing:0.05em; border-bottom:1px solid #30363d;">Active</th>
              <th style="text-align:left; padding:12px 16px; color:#8b949e; font-size:0.75rem; text-transform:uppercase; letter-spacing:0.05em; border-bottom:1px solid #30363d;">Sentiment</th>
            </tr>
          </thead>
  ...
  <script>
    ...
    function initAdminState() {
      // Initialize Roles array from localStorage or config.js
      let rCfg = localStorage.getItem('ITSM_ROLES_CONFIG_V2');
      if (rCfg) {
         adminRolesData = JSON.parse(rCfg);
      } else {
         // Transform old map to new array format
         const oldMap = window.CONFIG.role_sentiment_config || {};
         const roleNames = Object.keys(oldMap);
         const colors = ['#0078b4', '#5b21b6', '#a02818', '#e69500', '#00a86b', '#0078b4', '#00a86b', '#6f5091', '#00cccc', '#e69500', '#00a86b'];
      ...
      // Initialize Skills
      skillsData = {
         ea: localStorage.getItem('SKILL_ea') || ARCH_SYSTEM_PROMPT_BASE,
         sm: localStorage.getItem('SKILL_sm') || SM_SYSTEM_PROMPT_BASE,
         itsm: localStorage.getItem('SKILL_itsm') || DEFAULT_ITSM_SKILL
      };
      ...
      // Update global prompt vars immediately on boot
      rebuildGlobalPrompts();
    }
    
    function rebuildGlobalPrompts() {
      const itsmBase = localStorage.getItem('SKILL_itsm') || DEFAULT_ITSM_SKILL;
      const eaBase = localStorage.getItem('SKILL_ea') || ARCH_SYSTEM_PROMPT_BASE;
      const smBase = localStorage.getItem('SKILL_sm') || SM_SYSTEM_PROMPT_BASE;
...
" + itsmBase;
      ...
      let rCfg = localStorage.getItem('ITSM_ROLES_CONFIG_V2');
      if (rCfg) {
          const arr = JSON.parse(rCfg);
          const newMap = {};
          arr.forEach(r => newMap[r.role] = r.sentiment);
    ...
    // Override openAdmin to reset to dark UI
    openAdmin = function() {
    ...
    async function runPipeline(file) {
      ...
      try {
        ...
        pdfText = pages.join("\n\n").replace(/\b(COMM|EMAIL)\s*-\s*(\d+)\b/gi, '$1-$2');
...
Violating this rule breaks the data contract.`;
      ...
      let commManifest = '';
      if (commIdsInPdf.length > 35) {
        commManifest = `\n\n🚨 HIGH VOLUME TICKET DETECTED (${commIdsInPdf.length} COMM BLOCKS) 🚨
To prevent output truncation (JSON cutoff), apply the "Accordion" extraction method:
1. Extract the FIRST 5 and LAST 15 comments completely VERBATIM.
2. For the intermediate chronological comments, you MUST still create a timeline row, but aggressively SUMMARIZE the \`message_summary\` and \`detailed_activity_summary\` fields to save tokens.
Ensure valid JSON structure is maintained to the end.`;
      } else if (commIdsInPdf.length > 0) {
        commManifest = `\n\nMANDATORY COMM BLOCKS — every one of the following MUST appear as exactly one comment_added entry in the timeline. Extract their full content:\n${commIdsInPdf.join(', ')}`;
      }
...
- TIMELINE DETAIL: For each event, extract:
  ...
  * Message summary (if communication, extract the FULL message verbatim, unless high-volume rules apply)
  ...
  "timeline": [
    {
      ...
      "message_summary": "string — if this was a communication, extract the FULL message verbatim (unless high-volume rules apply)",
