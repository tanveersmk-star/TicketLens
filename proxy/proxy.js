...
// ── Configuration ──────────────────────────────────────────────────────────
const PORT          = 3001;
const MAX_BODY_BYTES = 20 * 1024 * 1024;  // 20 MB — expanded for large context inputs
...
const server = http.createServer((req, res) => {
  ...
  req.on('end', () => {
    ...
    const proxyReq = https.request(
      { ...UPSTREAM, method: 'POST', headers: upstreamHeaders, timeout: 300000 }, // 5 minute timeout
      proxyRes => {
    ...
    proxyReq.on('timeout', () => {
      proxyReq.destroy();
      sendError(res, 504, 'Upstream API request timed out.');
    });
